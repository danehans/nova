...
def zone_call(context, zone_name, method, **kwargs):
    """Route a call to a specific zone."""
...
def zone_cast(context, zone_name, method, **kwargs):
    """Route a cast to a specific zone."""
    routing_message = zones_common.form_routing_message(zone_name,
            'down', method, kwargs)
...
def get_all_zone_info(context):
    """Get the list of zones and their information from the manager."""
    msg = {'method': 'get_zone_info',
           'args': {}}
    return rpc.call(context, FLAGS.zones_topic, msg)
