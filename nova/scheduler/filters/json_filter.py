...
class JsonFilter(abstract_filter.AbstractHostFilter):
    ...
    def compute_host_passes(self, host_state, filter_properties):
        ...
        # FIXME(comstud):  I'm not completely thrilled with this, but
        # would prefer to keep this until we figure out what makes sense.
        # I think it might make sense that custom queries are specified
        # in the recently added 'options' stuff.  That would allow one to
        # modify filters without modifying code and restarting the
        # scheduler.
        ...
        if query:
            ...
            query = ['and', ['>=', '$free_ram_mb', required_ram],
                            ['>=', '$free_disk_mb', required_disk]]

        if not capabilities.get("enabled", True):
            return False
        result = self._process_filter(query, host_state)
