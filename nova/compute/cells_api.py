...
class ComputeCellsAPI(compute_api.API):
    ...
    def _cast_scheduler_message(self, *args, **kwargs):
        """In some cases we might super() and we don't want the parent
        class to try to send any messages to the scheduler directly.
        """
        return
    ...
    def _run_instance_rpc_method(self, context, topic, message):
        """Proxy run_instance rpc call to cells instead of scheduler"""
        args = message['args']
        cells_api.schedule_run_instance(context, **args)

    def _schedule_run_instance(self, rpc_method, *args, **kwargs):
        """Override default behavior and send this to cells service by
        passing in our own rpc method
        """
        super(ComputeCellsAPI, self)._schedule_run_instance(
