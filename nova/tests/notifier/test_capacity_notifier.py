...
class CapacityCacheNotifierTestCase(test.TestCase):
    """Test case for the CapacityCache updating notifier."""
    ...
    def test_update_called(self):
        def _verify_called(host, context, free_ram_mb, free_disk_gb, work,
                           vms):
            self.assertEquals(free_ram_mb, 123)
            self.assertEquals(free_disk_gb, 456)
            self.assertEquals(vms, -1)
            self.assertEquals(work, -1)

        self.stubs.Set(nova.db.api, "capacity_update", _verify_called)
